...
创建IGraphicBufferProducer和IGraphicBufferConsumer，以及FramebufferSurface，DisplayDevice对象。另外， 显示设备有3类：主设备，扩展设备，虚拟设备。其中前两个都是内置显示设备，故NUM\_BUILTIN\_DISPLAY\_TYPES=2，
...
HWComposer对象创建过程，会注册一些回调方法，当硬件产生VSYNC信号时，则会回调hook\_vsync()方法。
...
根据小节【2.8】可知监听BitTube，此处调用BitTube来sendObjects。一旦收到数据，则调用MQ.cb\_eventReceiver()方法。

#### 3.11.1 MQ.cb\_eventReceiver
...
![vsync](https://gityuan.com/images/surfaceFlinger/vsync.jpg)
...
3.  EventThread线程：执行到【小节3.11】DisplayEventReceiver::sendEvents()方法中调用BitTube::sendObjects()； 由【小节2.8】可知当收到数据则调用MQ.cb\_eventReceiver()，然后再经过handler消息机制，进入SurfaceFlinger主线程；
