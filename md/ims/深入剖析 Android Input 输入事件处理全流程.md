...
Android 系统中，输入事件的处理始于底层设备驱动。当用户操作输入设备（如触摸屏、键盘）时，设备驱动会将这些操作转换为相应的事件数据，并写入特定的文件描述符。

  

`EventHub`在系统启动时发挥重要作用，它会遍历`/dev/input`路径下的所有文件描述符（fd），并将其添加到`epoll`中。`epoll`是一种高效的 I/O 多路复用机制，能够同时监听多个文件描述符的事件。`EventHub`不仅负责监控现有设备的事件，还会实时监听新设备的创建和卸载。当驱动向描述符写入事件数据时，`epoll`被唤醒，`EventHub`通过`read`方法从描述符中读取原始事件，并将其简单封装成`rawEvent`，传递给`InputReader`。

  

`InputReader`运行在独立的线程中，其`threadLoop`方法会不断调用`EventHub`的`getEvents`来获取输入事件。获取到事件后，`InputReader`会通过`notifyxxx`方法将事件传递给`InputDispatcher`。`InputDispatcher`在接收到事件后，进一步通过`notifyxxx`方法将事件传递到上层，至此，输入事件完成了从底层设备到 Native 层的初步处理，即将开启向 Java 层传递的旅程。
...
文章中添加一些关于Android系统中输入事件处理机制的应用场景

继续完善文章中关于Android系统中输入事件处理机制的Native层处理部分

推荐一些关于Android系统中输入事件处理机制的优秀博客或文章