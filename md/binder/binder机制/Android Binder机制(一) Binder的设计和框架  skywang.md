---
created: 2025-06-06T16:05:02 (UTC +08:00)
tags: []
source: https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor2_1_1
author: skywang
---

# Android Binder机制(一) Binder的设计和框架 | skywang
...
> 
> **目录**  
> **1**. [Binder架构解析](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor1)  
> **1.1**. [Binder模型](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor1_1)  
> **1.2**. [Binder驱动存在的原因和意义](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor1_2)  
> **1.3**. [ServiceManager存在的原因和意义](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor1_3)  
> **1.4**. [为什么采用Binder机制，而不是其他的IPC通信方式](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor1_4)  
> **1.5**. [Binder中各角色之间关系](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor1_5)  
> **2**. [Binder设计解析](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor2)  
> **2.1**. [Binder设计](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor2_1)  
> **2.1.1**. [内核空间的Binder设计](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor2_1_1)  
> **2.1.2**. [用户空间的Binder设计](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor2_1_2)  
> **2.2**. [Binder通信](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor2_2)  
> **2.2.1**. [Binder通信模型](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor2_2_1)  
> **2.2.2**. [Binder通信数据](https://wangkuiwu.github.io/2014/09/01/Binder-Introduce/#anchor2_2_2)

## 1\. Binder架构解析

## 1.1 Binder模型

[![](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_frame.jpg)](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_frame.jpg)
...
通过上面的"Linux的内存划分"和"进程"，我们可以了解到：**应用程序都运行在用户空间，每个应用程序都有它自己独立的内存空间；若不同的应用程序之间涉及到通信，需要通过内核进行中转，因为需要用到内核的copy\_from\_user()和copy\_to\_user()等函数。**  
...
[![](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_4relationship.jpg)](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_4relationship.jpg)
...
说到Binder实体，就不得不说"Binder引用"。所谓Binder引用，实际上是内核中binder\_ref结构体的对象，它的作用是在表示"Binder实体"的引用。换句话说，每一个Binder引用都是某一个Binder实体的引用，通过Binder引用可以在内核中找到它对应的Binder实体。  
...
内核空间的Binder设计涉及到3个非常重要的结构体：binder\_proc，binder\_node和binder\_ref。由于本文的重点是介绍Binder机制的理论知识，因此，在这里我并不打算展开这3个结构体对它们进行详细介绍。当然，后面会再撰文对这些类进行详细说明。这里只需要了解个大概即可。

binder\_proc是描述进程上下文信息的，每一个用户空间的进程都对应一个binder\_proc结构体。  
binder\_node是Binder实体对应的结构体，它是Server在Binder驱动中的体现。  
binder\_ref是Binder引用对应的结构体，它是Client在Binder驱动中的体现。
...
如上图所示，binder\_proc中包含了3棵红黑树。  
(01) Binder实体红黑树是保存"binder\_proc对应的进程"所包含的Binder实体的，而Binder实体是与Server的服务对应的。可以将Binder实体红黑树理解为Server进程中包行的Server服务的红黑树。  
...
[![](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_kernel_ds02.jpg)](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_kernel_ds02.jpg)
...
[![](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_user_ds01.jpg)](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_user_ds01.jpg)
...
[![](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_communication.jpg)](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_communication.jpg)
...
[![](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_data.jpg)](https://raw.githubusercontent.com/wangkuiwu/android_applets/master/os/pic/binder/binder_data.jpg)

上面是用户空间和内核空间进行交互时，数据的打包方式。例如，当Client向Server发送请求时，Client会将数据打包成上诉格式，然后通过ioctl()发送给Binder驱动。根据数据的层次，从外到里分为3层进行说明。

**第一层**：这是用户空间的进程调用ioctl(fd,BINDER\_WRITE\_READ,&bwr)时传递给Binder驱动的信息。fd是Binder驱动的文件句柄，BINDER\_WRITE\_READ是ioctl()的一个标识，而bwr是传递的数据，它对应是途中的binder\_write\_read结构体的指针。binder\_write\_read中以write_开头的是保存请求数据的，而read_开头的是保存反馈数据的。其中，write\_size是请求数据的大小，write\_buffer是请求数据的内容，而write\_consumed是用来记录请求数据中已经被Binder驱动处理过的数据的大小。

**第二层**：这层的数据是"事务指令"+"binder\_transaction\_data结构体"组成的。图中给出的事务指令是BC\_TRANSACTION，表示该事务是请求；如果是回复，则是BR\_开头的，例如BR\_TRANSACTION。binder\_transaction\_data是描述事务交互数据的结构体；例如，target是指定事务目标，用来表示这个事务是交给谁进行处理的；code是事务编码，用来表示这是一个什么样的事务(例如，注册服务事务/获取服务事务等待)；data是保存事务中具体数据的内存地址。
