...
```
 mBehavior = BottomSheetBehavior.from(mBinding.bottomSheet);
        ...
        list1 = createItems();
        adapter = new ItemAdapter(list1, new ItemAdapter.OnItemClickListener() {
            @Override
            public void onclick(View v, int pos) {
                Logd.Logd(list1.get(pos).getTitle());
            }
        });
        mBinding.recyclerview.setAdapter(adapter);
        mBinding.btnWindow.setOnClickListener(new View.OnClickListener() {
            @Override
...
// 当控件在滑动的时候， ViewDragHelper拦截MotionEvent.ACTION_MOVE
...
> 这次需求是需要有悬浮底部的tab，可以使用peekHeight来显示底部。但是高度必须和底部设置一样。
>
> 因为状态比较多，且其中Hide_sate 会将内容整个隐藏。故需要关闭hideBehavior
>
> 除此之外，在OnViewRelease()中处理的高度，必须进行调整。 
>
> 需要重写botomSheetBehavior , 其中onViewRelease()方法，重新计算设置高度
